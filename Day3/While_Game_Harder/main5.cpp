...
//Declare variables
  ...
  int guess;
  int noOfGuesses;
  bool FLAG_continue = true; //set to 1

//Initalize guess
  guess = -1; // set to something out of range to make sure random number isn't set to guess.
  noOfGuesses =1; // 1, 2, 3, 4, 5, 6
...
//Generate the random number
  ...
  // cout << number; // Just used to see the number to verify code

  while ((FLAG_continue) && (noOfGuesses < 7)) // is FLAG_Continue still 1? Eventually it should be updated to 0
  {
//Prompt user to guess
   cout << "try to guess the number betwene 0-99 enter an int:" << endl;
      cin >> guess;
   cout << "\nYou guessed " << guess << endl;
   cout << "This is guess number " << noOfGuesses << " There is 6 total." << endl;
...
// Check to see if number is corerct   
    ...
    noOfGuesses++; //(Add 1 every guess increment)
    } 
}